...
# Use a dedicated environment variable for the shopping interface agent so that its identifier
# remains unique even if other shopping-related agents are running.
agentID = os.getenv("CORAL_SHOPPING_INTERFACE_AGENT_ID", "shopping-interface-agent")
params = {
    #"waitForAgents": 2,
    "agentId": agentID,
    "agentDescription": "You are user_interaction_agent for a shopping application, handling user instructions and coordinating tasks."
}

query_string = urllib.parse.urlencode(params)
MCP_SERVER_URL = f"{base_url}?{query_string}"
AGENT_NAME = "shopping_interaction_agent"
...
async def create_interface_agent(client, tools):
    ...
    prompt = ChatPromptTemplate.from_messages([
        (
            ...
            IMPORTANT: Only interact with agents whose identifier (agentId) contains the text "shopping-voice-agent". Ignore any agent that is not related to the shopping domain (e.g. restaurant agents).
            ...
            * Immediately relay each received response to the user using `ask_human` so the user stays informed in real-time.
